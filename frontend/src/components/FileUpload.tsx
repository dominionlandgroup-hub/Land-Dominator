  ...
  return (
    <label className="block cursor-pointer">
      <div
        ...
        style={{
          borderColor: dragging ? '#5C2977' : loading ? '#E8E0F0' : '#C4A8D8',
          background: dragging ? 'rgba(92,41,119,0.03)' : loading ? 'rgba(248,246,251,0.5)' : '#FDFBFF',
        ...
        onMouseLeave={(e) => {
          if (!loading && !dragging)
            (e.currentTarget as HTMLElement).style.borderColor = '#C4A8D8'
        }}
      >
        {loading ? (
          ...
          <div className="flex flex-col items-center gap-3">
            ...
            {selectedFile ? (
              <div className="flex items-center gap-2 px-4 py-2 rounded-lg" style={{ background: 'rgba(45,122,79,0.06)', border: '1px solid rgba(45,122,79,0.15)' }}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#2D7A4F" strokeWidth="2">
                  <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/>
                  <polyline points="14 2 14 8 20 8"/>
                </svg>
                <span className="text-xs font-medium truncate max-w-[200px]" style={{ color: '#2D7A4F' }}>{selectedFile}</span>
              </div>
            ) : (
              <div>
            ...
            <p className="text-xs" style={{ color: '#6B5B8A' }}>CSV files only · Max 300 MB</p>
          </div>
        )}
      </div>
      <input
