...
export default function MatchMap({ targets, comps = [], radiusMiles = 10, onSelectionChange }: Props) {
  ...
  function downloadSelected() {
    ...
    const headers = ['APN', 'Owner Name', 'Mail Address', 'City', 'State', 'ZIP', 'Acres', 'Score', 'Acreage Band', 'Confidence', 'Comp Count', 'Retail Estimate', 'Offer Low', 'Offer Mid', 'Offer High', 'Median Comp Sale Price', 'Median PPA', 'Min Comp Price', 'Max Comp Price', 'Outliers Removed', 'TLP Estimate', 'TLP Capped', 'Pricing Flag', 'No Match Reason']
    const rows = selectedParcels.map((p) => [
      ...
      p.outliers_removed ?? 0, p.tlp_estimate ?? '', p.tlp_capped ? 'Yes' : 'No', p.pricing_flag ?? '', p.no_match_reason ?? '',
