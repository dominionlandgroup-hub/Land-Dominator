...
const STAGES: StageConfig[] = [
  { id: 'lead',           label: 'Lead',           color: '#5C2977', bg: 'rgba(92,41,119,0.06)',   border: 'rgba(92,41,119,0.15)'  },
  { id: 'prospect',       label: 'Prospect',       color: '#4A90D9', bg: 'rgba(74,144,217,0.06)',  border: 'rgba(74,144,217,0.15)' },
  { id: 'offer_sent',     label: 'Offer Sent',     color: '#5C2977', bg: 'rgba(92,41,119,0.06)',   border: 'rgba(92,41,119,0.15)'  },
  { id: 'under_contract', label: 'Under Contract', color: '#059669', bg: 'rgba(5,150,105,0.06)',   border: 'rgba(5,150,105,0.15)'  },
  { id: 'due_diligence',  label: 'Due Diligence',  color: '#D97706', bg: 'rgba(217,119,6,0.06)',   border: 'rgba(217,119,6,0.15)'  },
  { id: 'closed_won',     label: 'Closed Won',     color: '#059669', bg: 'rgba(5,150,105,0.06)',   border: 'rgba(5,150,105,0.15)'  },
  { id: 'closed_lost',    label: 'Closed Lost',    color: '#DC2626', bg: 'rgba(220,38,38,0.06)',   border: 'rgba(220,38,38,0.15)'  },
]
      ...
      <div className="p-6">
        ...
        {loading ? (
          <div className="flex items-center justify-center py-16">
            <div className="text-sm" style={{ color: '#9B8AAE' }}>Loading deals…</div>
          </div>
        ) : (
                  ...
                  <div className="space-y-2">
                    ...
                    <button
                      style={{
                        ...
                        border: '1.5px dashed #E8E0F0', background: 'transparent', cursor: 'pointer',
                      }}
                      onMouseEnter={e => {
...
function DealCard({
  ...
  return (
    <div className="rounded-lg p-3 relative" style={{ background: '#FFFFFF', border: '1px solid #E8E0F0', boxShadow: '0 1px 3px rgba(92,41,119,0.08)' }}>
        ...
        <div className="relative flex-none" ref={menuRef}>
          ...
          {showMenu && (
            <div className="absolute right-0 top-full mt-1 rounded-lg z-30 overflow-hidden"
              style={{ background: '#FFFFFF', border: '1px solid #E8E0F0', minWidth: '170px', boxShadow: '0 1px 3px rgba(92,41,119,0.08)' }}>
              <div className="p-1">
              ...
              <div style={{ borderTop: '1px solid #E8E0F0' }} className="p-1">
                <button
      ...
      {deal.expected_close_date && (
        <div className="text-xs" style={{ color: '#6B5B8A' }}>
          Close: {new Date(deal.expected_close_date + 'T00:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
        </div>
      )}
      {deal.notes && (
        <div className="text-xs mt-1" style={{ color: '#9B8AAE', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
          {deal.notes}
        </div>
      )}
      {(deal.tags || []).length > 0 && (
        <div className="flex gap-1 flex-wrap mt-2">
          {(deal.tags || []).slice(0, 3).map(t => (
            <span key={t} className="px-1.5 py-0.5 rounded text-[10px]"
              style={{ background: 'rgba(92,41,119,0.08)', color: '#5C2977' }}>{t}</span>
          ))}
        </div>
      )}
    </div>
...
function DealForm({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(26,10,46,0.5)' }}>
      <div className="rounded-lg p-6 w-full" style={{ maxWidth: '440px', background: '#FFFFFF', border: '1px solid #E8E0F0', boxShadow: '0 1px 3px rgba(92,41,119,0.08)' }}>
        ...
        {error && (
          <div className="mb-3 p-3 rounded-lg text-sm" style={{ background: '#FEE2E2', color: '#DC2626', border: '1px solid rgba(220,38,38,0.3)' }}>
            {error}
          </div>
        )}
          ...
          <div>
            ...
            <textarea
              ...
              style={{
                ...
                background: '#F7F3FC', border: '1.5px solid #E8E0F0',
              ...
              <div className="flex gap-2 mt-2 flex-wrap">
                {(form.tags || []).map(t => (
                  <span key={t} className="flex items-center gap-1.5 px-2 py-1 rounded-lg text-xs"
                    style={{ background: 'rgba(92,41,119,0.08)', color: '#5C2977', border: '1px solid rgba(92,41,119,0.15)' }}>
                    {t}
